...
def test_patient_plasma_rejected():
    r = assess_sample_material(_item(title="Plasma proteomics from cancer patients TMT10"))
    assert r["qc_status"] == "rejected"
    assert "biofluid" in r["material_signals"]["excluded"]


def test_tissue_with_plasma_still_candidate():
    r = assess_sample_material(
        _item(title="FFPE tumor tissue and matched plasma TMT10 from patients")
    )
    assert r["qc_status"] == "candidate"
